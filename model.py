...
class ImageCaptionModel(tf.keras.Model):
    ...
    def train(self, train_captions, train_image_features, padding_index, batch_size=30):
        ...
        shuffled_indices = tf.random.shuffle(tf.range(train_captions.shape[0]))
        captions_shuffled = tf.gather(train_captions, shuffled_indices)
        image_features_shuffled = tf.gather(train_image_features, shuffled_indices)

        total_loss = total_seen = total_correct = 0
        for index, end in enumerate(range(batch_size, len(captions_shuffled)+1, batch_size)):
